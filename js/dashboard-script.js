...
// ══ STATE ═════════════════════════════════════════════════════════
let expenses         = [];
let banktxns         = [];   // BUG 2 FIX: was missing entirely
let accounts         = [];
let cats             = [];   // fetched from Firestore
let subcats          = {};   // BUG 1 FIX: was [] — must be {} (object keyed by cat name)
...
function saveCats() {
  ...
  if (!uid) return;
  db.collection('users').doc(uid).collection('settings').doc('categories')
    .set({ cats })
...
function saveSubcats() {
  const uid = auth.currentUser?.uid;
  if (!uid) return;
  db.collection('users').doc(uid).collection('settings').doc('subcategories')
    .set({ subcats })
    .catch(() => showToast('Failed to save subcategories'));
}
...
// ══ CLOUD CATEGORY LOADER ═════════════════════════════════════════
async function loadCatsFromCloud(uid) {
  const settingsRef = db.collection('users').doc(uid).collection('settings');

  const catDoc = await settingsRef.doc('categories').get();
  if (catDoc.exists && catDoc.data().cats && catDoc.data().cats.length > 0) {
    cats = catDoc.data().cats;
  } else {
    cats = DEFAULTS.map(d => ({...d}));
    await settingsRef.doc('categories').set({ cats });
  }

  const subDoc = await settingsRef.doc('subcategories').get();
  if (subDoc.exists && subDoc.data().subcats) {
    subcats = subDoc.data().subcats;
    // Merge defaults for any new default categories not yet in cloud
    Object.entries(DEFAULT_SUBCATS).forEach(([k, v]) => {
      if (!subcats[k]) subcats[k] = [...v];
    });
  } else {
    subcats = Object.fromEntries(
      Object.entries(DEFAULT_SUBCATS).map(([k,v]) => [k,[...v]])
    );
    await settingsRef.doc('subcategories').set({ subcats });
  }
...
// ══ REAL-TIME LISTENERS ═══════════════════════════════════════════
function listenForCats(uid) {
  const settingsRef = db.collection('users').doc(uid).collection('settings');

  settingsRef.doc('categories').onSnapshot(doc => {
  ...
  settingsRef.doc('subcategories').onSnapshot(doc => {
    if (doc.exists && doc.data().subcats) {
...
function listenForExpenses(uid) {
  listenForCats(uid); // starts real-time cat sync for other devices

  db.collection('users').doc(uid).collection('expenses')
    .orderBy('timestamp','desc')
    .onSnapshot(snap => {
      expenses = snap.docs.map(doc => ({id: doc.id, ...doc.data()}));
      renderRecent(); // BUG 4 FIX: now defined below
      if (currentView === 'analytics') renderAnalytics();
    });
}

// BUG 2 FIX: function was missing entirely
function listenForBankTxns(uid) {
  db.collection('users').doc(uid).collection('banktxns')
    .orderBy('timestamp','desc')
    .onSnapshot(snap => {
...
function listenForAccounts(uid) {
  db.collection('users').doc(uid).collection('accounts')
    .orderBy('timestamp','desc')
    .onSnapshot(snap => {
...
function renderAccounts() {
  ...
  // Rebuild dropdown — preserve selection if still valid
  const prevVal = sel.value;
  sel.innerHTML = '<option value="">Select account...</option>' +
    accounts.map(a => `<option value="${a.id}">${a.dispName}</option>`).join('');
  ...
  if (!accounts.length) {
    ...
    list.innerHTML = accounts.map(a => `
      <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;
                  background:var(--surface2);border-radius:10px;border:1px solid var(--border);margin-bottom:8px;">
        <div>
          <div style="font-size:13px;font-weight:600;color:var(--text)">${a.dispName}</div>
          <div style="font-size:11px;color:var(--muted);margin-top:2px;">${a.holderName} · ****${String(a.accNum).slice(-4)}</div>
        </div>
        <button class="pill-x" onclick="deleteAccount('${a.id}')"
                style="width:22px;height:22px;font-size:14px;">×</button>
      </div>`).join('');
  }
}
...
function deleteAccount(id) {
  ...
  db.collection('users').doc(auth.currentUser.uid).collection('accounts').doc(id).delete()
...
function renderSubcatGrid() {
  ...
  html += `<button class="add-pill-btn" title="Add subcategory" onclick="showAddSubcat()"
...
function addExpense() {
  ...
  const dd      = document.getElementById('inp-dd').value;
  const mm      = document.getElementById('inp-mm').value;
  const yyyy    = document.getElementById('inp-yyyy').value;
...
// ══ RECENT EXPENSES — BUG 4 FIX: renderRecent now defined ════════
// Called by the Firestore snapshot — refreshes the modal list if open
...
function openRecentModal() {
  document.getElementById('recent-modal').classList.add('open');
  renderRecentModalList();
}
...
document.getElementById('recent-modal').addEventListener('click', function(e) {
  if (e.target === this) closeRecentModal();
});
...
function renderRecentModalList() {
  ...
  list.innerHTML = expenses.map(e => {
    ...
    <div class="expense-item" style="cursor:default">
      <input type="checkbox" class="exp-check" value="${e.id}"
             style="width:18px;height:18px;accent-color:var(--danger);cursor:pointer;flex-shrink:0;">
...
function deleteSelectedExpenses() {
  ...
  if (!checkboxes.length) return showToast('Select at least one expense');
  if (!confirm(`Delete ${checkboxes.length} selected expense(s)?`)) return;

  const user     = auth.currentUser;
  const promises = Array.from(checkboxes).map(cb =>
    db.collection('users').doc(user.uid).collection('expenses').doc(cb.value).delete()
  );
  Promise.all(promises)
    .then(() => showToast(`${checkboxes.length} expense(s) deleted`))
...
function renderCatFilterRow() {
  ...
  html += cats.map(c => {
    ...
    return `<button class="cat-filter-pill${active?' active':''}" ${style}
              onclick="toggleCatFilter('${c.name.replace(/'/g,"\\'")}')">
      ${c.icon} ${c.name}
    </button>`;
  }).join('');
  row.innerHTML = html;
...
function renderSubcatFilterRow() {
  ...
  let html = `<button class="cat-filter-pill all-pill${allActive?' active':''}" style="font-size:10px"
                onclick="toggleSubcatFilter('__ALL__')">All Subcategories</button>`;
  html += availSubs.map(s => {
    ...
    return `<button class="cat-filter-pill${active?' active':''}" ${style} style="font-size:10px"
              onclick="toggleSubcatFilter('${s.replace(/'/g,"\\'")}')">
      ${s}
    </button>`;
  }).join('');
  row.innerHTML = html;
...
function toggleCatFilter(name) {
  if (name==='__ALL__') { selCatFilters = new Set(['__ALL__']); }
    ...
    selCatFilters.delete('__ALL__');
    if (selCatFilters.has(name)) selCatFilters.delete(name); else selCatFilters.add(name);
    if (selCatFilters.size===0) selCatFilters = new Set(['__ALL__']);
  }
...
function toggleSubcatFilter(name) {
  if (name==='__ALL__') { selSubcatFilters = new Set(['__ALL__']); }
    ...
    selSubcatFilters.delete('__ALL__');
    if (selSubcatFilters.has(name)) selSubcatFilters.delete(name); else selSubcatFilters.add(name);
    if (selSubcatFilters.size===0) selSubcatFilters = new Set(['__ALL__']);
  }
  renderSubcatFilterRow();
  renderAnalytics();
}
...
function getActiveCatNames() {
  return selCatFilters.has('__ALL__') ? cats.map(c => c.name) : [...selCatFilters];
}
...
function setPeriod(p, btn) {
  ...
  if (p==='custom')  {
    document.getElementById('custom-picker').style.display='flex'; dateNav.style.display='none';
    const v = parseInt(document.getElementById('inp-custom-days').value);
    if (v>0) applyCustomDays();
    else { document.getElementById('date-label').textContent='Enter number of days'; customRangeStart=''; customRangeEnd=''; }
    return;
  }
...
function useDefaultWeek() {
  ...
  document.getElementById('wk-from-dd').value   = mon.getDate();
  document.getElementById('wk-from-mm').value   = mon.getMonth()+1;
  document.getElementById('wk-from-yyyy').value = mon.getFullYear();
  document.getElementById('wk-to-dd').value     = sun.getDate();
  document.getElementById('wk-to-mm').value     = sun.getMonth()+1;
  document.getElementById('wk-to-yyyy').value   = sun.getFullYear();
  customRangeStart = mon.toISOString().slice(0,10);
  customRangeEnd   = sun.toISOString().slice(0,10);
  renderAnalytics();
}
...
function applyYearPicker() {
  ...
  customRangeStart=`${yr}-01-01`; customRangeEnd=`${yr}-12-31`; renderAnalytics();
}
...
function getPeriodDates() {
  ...
  if (currentPeriod==='weekly'||currentPeriod==='monthly'||currentPeriod==='yearly'||currentPeriod==='custom') {
    ...
    const label = currentPeriod==='custom'
      ? (document.getElementById('inp-custom-days').value ? `Last ${document.getElementById('inp-custom-days').value} days` : '—')
...
function renderAnalytics() {
  ...
  const filtered      = expenses.filter(e =>
    ...
    activeCats.has(e.cat) &&
    (!activeSubcats || !e.subcat || activeSubcats.has(e.subcat))
  );
  ...
  if (catList.length) {
    document.getElementById('stat-top').textContent     = catList[0][0];
    document.getElementById('stat-top-amt').textContent = '₹' + catList[0][1].toLocaleString('en-IN');
  } else {
  ...
  const legendClr= isDark ? '#6a6a80' : '#888890';
  const brdClr   = isDark ? '#141418' : '#ffffff';
  const labels   = catList.map(c=>c[0]);
  const data     = catList.map(c=>c[1]);
  ...
  if (!data.length) {
    ...
    ctx.fillStyle=legendClr; ctx.font='14px Inter'; ctx.textAlign='center';
    ctx.fillText('No data for this period', ctx.canvas.width/2, ctx.canvas.height/2);
  } else {
    pieChart = new Chart(ctx, {
      type: 'doughnut',
      data: { labels, datasets:[{ data, backgroundColor:colors, borderColor:brdClr, borderWidth:3, hoverOffset:8 }] },
      options: {
        responsive:true, cutout:'60%',
        plugins:{
          legend:{ position:'bottom', labels:{ color:legendClr, font:{family:'Inter',size:11}, padding:12, boxWidth:9, boxHeight:9 } },
  ...
  if (!catList.length) {
    bd.innerHTML = '<div class="empty-state" style="padding:16px"><div class="empty-icon">📊</div>No data yet</div>';
  } else {
    bd.innerHTML = catList.map(([cat,amt]) => {
      const color=getCatColor(cat), pct=total?Math.round((amt/total)*100):0;
      return `<div class="breakdown-row">
        <div class="bd-dot" style="background:${color}"></div>
        <div class="bd-cat">${getCatIcon(cat)} ${cat}</div>
...
function renderTrendTable(activeCats, curStart) {
  ...
  const rows=buckets.map(b => {
    ...
    const total=exps.reduce((s,e)=>s+e.amount,0), cnt=exps.length, perCat={};
    catNames.forEach(n => { perCat[n]=exps.filter(e=>e.cat===n).reduce((s,e)=>s+e.amount,0); });
    return {...b, total, cnt, perCat};
  });
  const maxTotal=Math.max(...rows.map(r=>r.total),1);
  let catCols='';
  if (!showAll && catNames.length>1) catCols=catNames.map(n=>`<th>${getCatIcon(n)} ${n}</th>`).join('');
  ...
  const tbody=rows.map(r => {
    const pct=maxTotal?Math.round((r.total/maxTotal)*100):0, barColor=r.total===maxTotal?'var(--accent)':'var(--muted)';
    let catTds='';
    if (!showAll&&catNames.length>1) catTds=catNames.map(n=>`<td class="amt">${r.perCat[n]?'₹'+r.perCat[n].toLocaleString('en-IN'):'-'}</td>`).join('');
    return `<tr><td>${r.label}</td>${catTds}<td class="amt">₹${r.total.toLocaleString('en-IN')}</td><td class="cnt">${r.cnt}</td>
      <td><div style="height:6px;border-radius:4px;background:var(--border);overflow:hidden">
...
function openExportModal() {
  ...
  document.getElementById('exp-from-dd').value   = firstDay.getDate();
  document.getElementById('exp-from-mm').value   = firstDay.getMonth()+1;
  document.getElementById('exp-from-yyyy').value = firstDay.getFullYear();
  document.getElementById('exp-to-dd').value     = now.getDate();
  document.getElementById('exp-to-mm').value     = now.getMonth()+1;
  document.getElementById('exp-to-yyyy').value   = now.getFullYear();
...
function renderExpCatFilter() {
  ...
  html+=cats.map(c => {
    const active=!allActive&&expSelCats.has(c.name);
    const style=active?`style="background:${c.color}"`:'';
    return `<button class="cat-filter-pill${active?' active':''}" ${style} onclick="toggleExpCat('${c.name.replace(/'/g,"\\'")}')"> ${c.icon} ${c.name} </button>`;
  }).join('');
  wrap.innerHTML=html;
}
...
function parseModalDate(pfx) {
  const dd=parseInt(document.getElementById(`exp-${pfx}-dd`).value,10);
  const mm=parseInt(document.getElementById(`exp-${pfx}-mm`).value,10);
  const yyyy=parseInt(document.getElementById(`exp-${pfx}-yyyy`).value,10);
...
function drawTableHeader(doc,ML,y,PW,MR,MUTED) {
  ...
  doc.text('CATEGORY',ML+28,  y+5);
  doc.text('SUBCAT',  ML+58,  y+5);
  doc.text('NOTE',    ML+86,  y+5);
  doc.text('AMOUNT',  PW-MR-2,y+5, {align:'right'});
}
...
function runExportPDF() {
  ...
  const filtered=expenses.filter(e=>e.date>=fromIso&&e.date<=toIso&&activeCats.has(e.cat))
  ...
  doc.setFillColor(...ACCENT); doc.roundedRect(ML,y,CW,18,3,3,'F');
  doc.setFont('helvetica','bold'); doc.setFontSize(13); doc.setTextColor(...WHITE);
  doc.text('EXPENSE MANAGER - Expense Report',ML+6,y+11); y+=22;

  doc.setFontSize(8); doc.setFont('helvetica','normal'); doc.setTextColor(...MUTED);
  doc.text('Period: '+formatDate(fromIso)+'  to  '+formatDate(toIso),ML,y);
  doc.text('Generated: '+formatDate(new Date().toISOString().slice(0,10)),PW-MR,y,{align:'right'});
  y+=5; doc.setDrawColor(...BORDER); doc.setLineWidth(0.3); doc.line(ML,y,PW-MR,y); y+=7;

  const total=filtered.reduce((s,e)=>s+e.amount,0), bycat={};
  filtered.forEach(e=>{bycat[e.cat]=(bycat[e.cat]||0)+e.amount;});
  const topCat=Object.entries(bycat).sort((a,b)=>b[1]-a[1])[0];
  const avg=filtered.length?Math.round(total/filtered.length):0;
  const boxes=[
    {label:'TOTAL SPENT',  value:fmtRs(total)},
    {label:'TRANSACTIONS', value:String(filtered.length)},
    {label:'AVG / ENTRY',  value:fmtRs(avg)},
    {label:'TOP CATEGORY', value:topCat?safeStr(topCat[0]):'N/A'},
  ];
  const bw=(CW-6)/4;
  boxes.forEach((b,i)=>{
    const bx=ML+i*(bw+2);
    doc.setFillColor(245,245,242); doc.roundedRect(bx,y,bw,17,2,2,'F');
    doc.setDrawColor(...BORDER); doc.setLineWidth(0.25); doc.roundedRect(bx,y,bw,17,2,2,'S');
    doc.setFontSize(6.5); doc.setFont('helvetica','normal'); doc.setTextColor(...MUTED);
    doc.text(b.label,bx+bw/2,y+5.5,{align:'center'});
    doc.setFontSize(b.value.length>12?8:10); doc.setFont('helvetica','bold'); doc.setTextColor(...DARK);
  ...
  if (Object.keys(bycat).length>1) {
    ...
    doc.text('CATEGORY BREAKDOWN',ML,y); y+=4;
    doc.setDrawColor(...BORDER); doc.line(ML,y,PW-MR,y); y+=4;
    Object.entries(bycat).sort((a,b)=>b[1]-a[1]).forEach(([cat,amt])=>{
      const pct=total?Math.round((amt/total)*100):0, rgb=hexToRgb(getCatColor(cat));
      doc.setFillColor(...rgb); doc.circle(ML+2,y+1.8,1.8,'F');
      doc.setFontSize(8); doc.setFont('helvetica','normal'); doc.setTextColor(...DARK);
      doc.text(safeStr(cat),ML+6,y+3.5);
      const barX=ML+54,barW=76,barH=3.5;
      doc.setFillColor(...BORDER); doc.roundedRect(barX,y+0.5,barW,barH,1,1,'F');
      if (pct>0) { doc.setFillColor(...rgb); doc.roundedRect(barX,y+0.5,barW*(pct/100),barH,1,1,'F'); }
      doc.setFontSize(8); doc.setFont('helvetica','bold'); doc.setTextColor(...rgb);
      doc.text(fmtRs(amt),PW-MR-18,y+3.5,{align:'right'});
      doc.setTextColor(...MUTED); doc.text(pct+'%',PW-MR,y+3.5,{align:'right'});
      y+=8; if(y>PH-30){doc.addPage();y=MT;}
    });
    y+=3;
  }

  doc.setDrawColor(...BORDER); doc.line(ML,y,PW-MR,y); y+=4;
  drawTableHeader(doc,ML,y,PW,MR,MUTED); y+=9;

  let rowCount=0;
  filtered.forEach(e=>{
    if (y>PH-24) { doc.addPage(); y=MT; drawTableHeader(doc,ML,y,PW,MR,MUTED); y+=9; }
    if (rowCount%2===0) { doc.setFillColor(250,250,248); doc.rect(ML,y-1,CW,7.5,'F'); }
    const rgb=hexToRgb(getCatColor(e.cat));
    doc.setFillColor(...rgb); doc.circle(ML+2,y+2.8,1.5,'F');
    doc.setFontSize(8); doc.setFont('helvetica','normal'); doc.setTextColor(...DARK);
    doc.text(e.date?formatDate(e.date):'-', ML+5,  y+4.5);
    doc.text(safeStr(e.cat),               ML+28,  y+4.5);
    doc.text(safeStr(e.subcat||''),        ML+58,  y+4.5);
    doc.setTextColor(...MUTED);
    doc.text(safeNote(e.note,24),          ML+86,  y+4.5);
    doc.setFont('helvetica','bold'); doc.setTextColor(...rgb);
    doc.text(fmtRs(e.amount),             PW-MR-2,y+4.5,{align:'right'});
    y+=7.5; rowCount++;
  });

  y+=2; doc.setDrawColor(...BORDER); doc.line(ML,y,PW-MR,y); y+=6;
  doc.setFontSize(9.5); doc.setFont('helvetica','bold');
  doc.setTextColor(...DARK); doc.text('TOTAL',ML+5,y);
  doc.setTextColor(...ACCENT); doc.text(fmtRs(total),PW-MR-2,y,{align:'right'});

  const pageCount=doc.getNumberOfPages();
  for (let p=1;p<=pageCount;p++) {
    doc.setPage(p); doc.setFontSize(7); doc.setFont('helvetica','normal'); doc.setTextColor(...MUTED);
    doc.text('Page '+p+' of '+pageCount+' | Expense Manager',PW/2,PH-8,{align:'center'});
  }
  doc.save('expenses_'+fromIso+'_to_'+toIso+'.pdf');
  closeExportModal();
  showToast('PDF saved!');
}
